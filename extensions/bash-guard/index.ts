...
function hasFlag(args: string[], flag: string): boolean {
  return (
    args.includes(flag) ||
    args.some(
      (a) => a.startsWith(flag) && flag.length === 2 && a.startsWith("-"),
    )
  );
}
...
async function promptRunOrAbort(
  ...
  const choice = await ctx.ui.custom<"run" | "abort">(
    (tui: TUI, theme: ExtensionContext["ui"]["theme"], _kb, done) => {
      ...
      // paddingY=1 gives a blank line between the command body and the
      // Run/Abort list (wrapTextWithAnsi drops trailing newlines, so a
      // spacer must come from padding, not from the text itself).
...
const MAIN_DISABLED_BLOCKED: Array<{ pattern: RegExp; reason: string }> =
  HEADLESS_BLOCKED.filter(({ pattern }) => {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerFlag("bash-guard-disabled", {
    description:
      "Start the session with bash-guard disabled (autonomous mode; hard-block floor still applies).",
    type: "boolean",
    default: false,
  });
  ...
  pi.on("session_start", async (event, ctx) => {
    if (
      ...
      pi.getFlag("--bash-guard-disabled") === true
    ) {
      disabled = true;
      const { theme } = ctx.ui;
  ...
  pi.registerCommand("bash-guard", {
    description:
      "Toggle bash-guard between interactive (default) and disabled (autonomous) for this session.",
