...
export default function (pi: ExtensionAPI) {
  ...
  const protectedPaths = [
    { pattern: /\.env($|rc$|\.(?!example))/, desc: "environment file" }, // .env, .envrc, .env.local (but not .env.example)
  ...
  const softProtectedPaths = [
    ...
    { pattern: /bun\.lock$/, desc: "bun.lock" },
  ];

  const dangerousBashWrites = [
    />\s*\.env(rc)?(?!\.example)(\b|$)/, // echo x > .env, .envrc, .env.local (but not .env.example)
