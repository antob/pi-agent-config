...
const SUPPORTED_TYPES = [
  ...
  "image/bmp", "image/x-bmp", "image/x-ms-bmp", "image/x-ms-bitmap"
];
...
async function exec(pi: ExtensionAPI, cmd: string): Promise<{ code?: number; stdout: string; stderr: string }> {
  return pi.exec("bash", ["-lc", `set -o pipefail; ${cmd}`], { timeout: 10000 }) as Promise<{
    code?: number;
    stdout: string;
    stderr: string;
  }>;
...
async function captureWayland(pi: ExtensionAPI, out: string): Promise<void> {
  ...
  if ((listResult.code ?? 0) !== 0) {
    throw new Error(`wl-paste --list-types failed: ${listResult.stderr.trim() || "unknown error"}`);
  }

  const types = listResult.stdout.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
  ...
  if ((result.code ?? 0) !== 0) {
    throw new Error(`Clipboard capture failed: ${result.stderr.trim() || result.stdout.trim() || "unknown error"}`);
  }
}
