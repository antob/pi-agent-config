 ...
 *
 * This extension provides the `/context` command in pi.
 * It displays a TUI overlay that visualizes the current session's
 * context usage, breaking down token consumption by categories such as
 * System Prompt, System Tools, Tool Calls, and Messages.
 */
import {
  type ExtensionAPI,
  type SessionManager,
    ...
    handler: async (_args, ctx) => {
      ...
      await ctx.ui.custom(
        (_tui, theme, _kb, done) => {
          ...
          const totalUsageTitle = `${theme.fg("text", theme.bold("Total Usage".padEnd(16)))} ${theme.fg("text", theme.bold(formatTokens(totalActual).padStart(7)))} ${theme.fg("text", theme.bold(`(${(usage.percent || 0).toFixed(1).padStart(5)}%)`))}`;
