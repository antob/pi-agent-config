...
export function renderEditDiffResult(
	...
	return {
		render(width: number): string[] {
			...
			const mode = resolveDiffPresentationMode(safeWidth, canRenderSplitLayout(safeWidth));
			if (
				cachedLines
...
export function renderWriteDiffResult(
	...
	return {
		render(width: number): string[] {
			const safeWidth = normalizeDiffRenderWidth(width);
			const resolvedMode = resolveDiffPresentationMode(safeWidth, canRenderSplitLayout(safeWidth));
