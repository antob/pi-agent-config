    ...
    (
      ...
      function render(width: number): string[] {
        ...
        for (let i = 0; i < allItems.length; i++) {
          ...
          // Blank line between the "Other" entry and the Submit row, so
          // Submit reads as a separate action and not as another option.
          if (item.isSubmit) lines.push("");
...
// a time, so ALL pop-up-style tools (ask_user, quiz, ...) must
...
export default function askUserQuestion(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "If you need answers to multiple questions, make multiple separate ask_user tool calls instead of combining them into one prompt.",
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      if (!ctx.hasUI) {
        return unavailableResult(
          params.question,
          mode,
          "ask_user requires interactive mode UI",
          context,
        );
      }
    ...
    renderResult(result, _options, theme) {
      ...
      if (details.status === "unavailable") {
        return new Text(
          theme.fg("warning", details.message || "ask_user unavailable"),
          0,
          0,
        );
      }
