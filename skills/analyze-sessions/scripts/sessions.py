...
def parent_session_id_from_path(path: Path) -> str | None:
...
class Filters:
    ...
    session_id: str | None = None  # exact or prefix
    include_subagents: bool | None = None  # tri-state; script default if None
    limit: int | None = None
    min_cost: float | None = None
    min_messages: int | None = None
    errors_only: bool = False
    grep: str | None = None  # substring on user text, case-insensitive
...
def add_filter_args(p: argparse.ArgumentParser, *, subagents_default: bool) -> None:
    ...
    p.add_argument(
        "--since",
        metavar="WHEN",
        help="Lower bound: YYYY-MM-DD, ISO datetime, or relative (7d/2w/3h/30m).",
    )
    p.add_argument(
        "--until", metavar="WHEN", help="Upper bound, same formats as --since."
    )
    p.add_argument(
        "--cwd",
        action="append",
...
def fmt_short_ts(dt: datetime | None) -> str:
    if not dt:
        return "?"
