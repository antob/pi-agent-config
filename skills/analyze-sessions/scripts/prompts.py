...
def _render_markdown(sess_prompts, total_prompts: int) -> None:
    ...
    proj_order = sorted(
        by_proj.keys(),
        key=lambda k: (
            max((s.started_at for s, _ in by_proj[k] if s.started_at), default=None)
            or 0
        ),
        reverse=True,
    )

    print("# Pi prompts dump")
    print(
        f"_{total_prompts} prompts across {len(sess_prompts)} sessions, "
    ...
    for proj in proj_order:
        ...
        for s, prompts in sorted(
            by_proj[proj], key=lambda x: x[0].started_at or 0, reverse=True
        ):
            print(
                f"### {S.fmt_short_ts(s.started_at)}  ·  `{s.short_id}`  "
