...
def main() -> int:
    ...
    S.stderr(
        f"# {total_hits} match(es) across {sessions_with_hits} session(s) "
...
def _search_session(
    ...
    for rec in S.iter_records(s.path):
        ...
        if role == "user" and scope in ("user", "both"):
            ...
            hits.extend(
                _collect(text, "user", matcher, finditer, context_lines, snippet_chars)
            )
...
def _collect(
    text: str, kind: str, matcher, finditer, context_lines: int, snippet_chars: int
):
