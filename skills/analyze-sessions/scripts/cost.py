...
def main() -> int:
    ...
    print()
    print_grouped(
        summaries, args.by, show_subagents=args.show_subagents, limit=group_limit
    )
    return 0
...
def session_keys(s: S.SessionSummary, by: str):
    ...
    if by == "day":
        k = (
            s.started_at.astimezone().strftime("%Y-%m-%d")
            if s.started_at
            else "unknown"
        )
...
def build_report(summaries, by: str) -> dict:
    rows = defaultdict(
        lambda: dict(
            cost=0.0, sessions=0, messages=0, errors=0, sub_cost=0.0, sub_sessions=0
        )
    )
    for s in summaries:
...
def print_grand_total(summaries) -> None:
    ...
    print(
        f"  sessions:  {len(top_level)} top-level"
...
def print_grouped(summaries, by: str, *, show_subagents: bool, limit) -> None:
    ...
    keycol = max(
        len(label), max((len(_render_key(k, by)) for k, _ in items), default=4)
    )
