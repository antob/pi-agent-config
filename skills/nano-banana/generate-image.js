...
async function main() {
  ...
  const outDir = resolve(process.cwd(), ".context/generated-images");
  mkdirSync(outDir, { recursive: true });
  const outPath = resolve(
    output || join(outDir, `nano-banana-${Date.now()}.png`),
  );

  const content = [{ type: "text", text: prompt }];
  if (inputImage) {
    const absInput = resolve(inputImage);
  ...
  if (Array.isArray(message?.content)) {
    for (const item of message.content) {
      ...
      if (item?.type === "output_image" && item?.image_url)
        images.push(item.image_url);
      if (item?.type === "output_image" && item?.output_image)
        images.push(item.output_image);
  ...
  if (!first) {
    ...
    if (!directBase64) {
      console.log(
        JSON.stringify(
          { ok: true, warning: "No image returned", response: json },
          null,
          2,
        ),
      );
  ...
  if (typeof chosen === "string" && chosen.startsWith("data:")) {
    ...
    chosen?.image_url?.url &&
    typeof chosen.image_url.url === "string" &&
    chosen.image_url.url.startsWith("data:")
  ) {
    base64 = chosen.image_url.url.split(",")[1] || "";
  } else if (
    chosen?.url &&
    typeof chosen.url === "string" &&
    chosen.url.startsWith("data:")
  ) {
  ...
  if (!base64) {
    console.log(
      JSON.stringify(
        {
          ok: true,
          warning: "Image payload format not recognized",
          response: json,
        },
        null,
        2,
